...
/*
 * TODO 4: child_fn()
 * FIX: exec config->command directly instead of /bin/sh
 */
int child_fn(void *arg)
{
    ...
    /* FIX: run the actual command, not /bin/sh */
...
static int run_supervisor(const char *rootfs)
{
    ...
    if (listen(ctx.server_fd, 5) < 0) { perror("listen"); goto cleanup; }
    ...
    while (!ctx.should_stop) {
        ...
        case CMD_RUN: {
            ...
            /* Print to supervisor terminal for Screenshot 1 */
            printf("[supervisor] Container started: %s (PID %d)\n",
                   client_req.container_id, child_pid);
            fflush(stdout);
        ...
        case CMD_STOP: {
            ...
            while (container != NULL) {
                if (strcmp(container->id, client_req.container_id) == 0) {
                    ...
                    printf("[supervisor] Container stopped: %s (PID %d)\n",
                           client_req.container_id, container->host_pid);
                    fflush(stdout);
...
static int send_control_request(const control_request_t *req)
{
    ...
    if (send(sock, req, sizeof(*req), 0) < 0) { perror("send"); close(sock); return 1; }

    nread = recv(sock, &response, sizeof(response), 0);
    if (nread < 0) { perror("recv"); close(sock); return 1; }
